...
```{r}
full_ratings_set %>% group_by(status_id) %>% mutate(rating = as.numeric(rating)) %>% summarise(mean = mean(rating), sd=sd(rating)) %>% mutate(zscore = scale(mean), sig = abs(mean)>=1.96*sd)
```
